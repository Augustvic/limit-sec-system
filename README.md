...
采用互斥锁解决此问题。缓存失效的时候，先去获取锁，获取到锁的可以去请求数据库，没有得到锁的不断重试。

“预减库存”之前需要判断库存是否已加载到缓存中...这些地方用到 redis 实现的互斥锁来避免缓存击穿的问题。
...
使用 Redis 中的 hash 数据结构存储用户的基本信息和会话信息。
...
但此处客户端对于单个用户信息的访问不存在高并发读取的情况，所以选择修改缓存即可。

#### 5.2.2 缓存与数据库一致性

先操作数据库，再操作缓存。

通常情况下建议先操作数据库再操作缓存，但此处客户端对于单个用户信息的访问不存在高并发读取的情况，可先修改缓存，保证客户端即刻就能读取到最新信息，然后再写入数据库。
